...
B<Shapebar> is based on B<lemonboy/bar> and B<krypt-n/bar> which provides the standard Lemonbar functionity along with Xft support.
Shapebar adds monitor cloning mode (-c), (todo: shaped borders) and removes clickable areas.
...
Shapebar adds shapes to draw in the backgound. They serve as seperators. This is how Shapebar derives it's name.
...
=item Clickable areas are removed because I didn't design shapebar to be interactive.
...
set the number of points in the curved part of the arc and sigmoid seperator. Must be a non-negative integer.
...
Draw a seperator. Use the format Z??. First char after the Z is the mode, which selects the seperator. Second char is the direction.
Possible modes are t and a. t Draws a triangle with an horizontal and a vertical edge, T draws a triangle with a vertical edge.
a draws and arc and s draws a sigmoid. Possible directions are < and > for left and right.
For example Zt> Draws a triangle seperator orientated to the right. Seperators respect the width, height, foreground and background colour formatting parameters.

=item B<W>

Sets the width parameter used for seperators.

=item B<H>

Sets the height parameter used for seperators.
